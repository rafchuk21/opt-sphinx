...
class GetCylindricalRegion(SubDomain):
    ...
    def inside(self, x, on_boundary):
        ...
        xam = (xva**2.0)/np.dot(a,a) # (x*a)^2/|a|^2
        dsq = xvm - xam # d^2 = |x|^2 - proj(x onto a)^2

        am = dot(a,a) # |a|^2

        # d^2 <= r^2 && 0 <= proj(x onto a)/|a| = (x*a)/|a|^2 <= 1
        return between(dsq, (0.0,r**2.0)) and between(xva/am, (0.0,1.0))
...
class GetPlanarBoundary(SubDomain):
    def __init__(self, p1:Point, p2:Point, p3:Point, p4:Point):
        ...
        maxdist = np.amax([v12m, v13m, v14m])
        # Find which diagonal in parallelogram is different
        if near(v12m, maxdist): # p2 is across from p1
            self.p2 = p3
            self.p3 = p2
            self.p4 = p4
        elif near(v13m, maxdist): # p3 is across from p1
            self.p2 = p2
            self.p3 = p3
            self.p4 = p4
        else:
    ...
    def inside(self, x, on_boundary):
        ...
        if near(x[2], 3.0) and x[0] > 13.5:
            print(x)
            print(A)
            print(s)
            print(mag_cross(v12, v14))
            print('--------------------')
    ...
    # TODO Verify parallelogram assumption
    @classmethod
